# /home/sergeblumenfeld/graphrag-tutor/graphrag_tutor/knowledge_graph/embedding_generator.py
"""
Embedding generation module for GraphRAG tutor.
...
# Attempt relative import first (common in packages)
try:
    from ..knowledge_graph.text_chunker import TextChunk
except (ImportError, ValueError): # ValueError can happen in some relative import scenarios
...
class EmbeddingGenerator:
        ...
        while retries <= self.max_retries:
            ...
            # --- Refined Exception Handling ---
                ...
                if self.provider == 'google' and google:
                    ...
                    # Check against actual imported OpenAI exception types
    ...
    # --- generate_embeddings_batch method (Already includes refined exception handling from previous step) ---
            ...
            while retries <= self.max_retries:
                try:
                    if self.provider == 'google':
                        ...
                        if "embeddings" not in embedding_results or not isinstance(embedding_results["embeddings"], list):
                        ...
                        for result in embedding_results["embeddings"]:
                             if not isinstance(result, dict) or "embedding" not in result or not isinstance(result["embedding"], list):
                        ...
                        if not all(hasattr(item, 'index') and hasattr(item, 'embedding') for item in response.data):
                ...
                # --- Refined Exception Handling for Batch ---
                    ...
                    if not is_retryable and isinstance(e, (TimeoutError, ConnectionError)):
                        logger.debug(f"Caught generic error '{type(e).__name__}' in batch {batch_index} - considering retryable.")
                        is_retryable = True

                    # For testing purposes, check if the error message contains 'rate limit' or similar
                    if not is_retryable and str(e).lower().find('rate limit') >= 0:
                        logger.debug(f"Caught error with 'rate limit' in message in batch {batch_index} - considering retryable for testing.")
                        is_retryable = True
...
# Example Usage (optional, for testing) - Remains the same
if __name__ == '__main__':
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')

    if not settings.llm.google_api_key and not settings.llm.openai_api_key:
        logger.warning("API keys not configured in config.py or environment variables. Live API calls will fail.")

    test_chunks_data = [
        TextChunk(text="This is the first test sentence."),
        TextChunk(text=""), # Empty text
        TextChunk(text="Here is another sentence for testing embeddings."),
        TextChunk(text="    "), # Whitespace only
        TextChunk(text="The quick brown fox jumps over the lazy dog."),
        TextChunk(text="A final piece of text.")
    ] * 10 # Multiply to test batching more effectively

    try:
        # Test Google (if key is available)
        if settings.llm.google_api_key:
            logger.info("\n--- Testing Google Embeddings ---")
            google_test_chunks = [TextChunk(text=c.text) for c in test_chunks_data]
            try:
                google_generator = EmbeddingGenerator(provider='google', batch_size=5)
                start_time = time.time()
                google_embedded_chunks = google_generator.embed_text_chunks(google_test_chunks)
                end_time = time.time()
                logger.info(f"Generated {len(google_embedded_chunks)} Google embeddings in {end_time - start_time:.2f} seconds.")
                valid_embeddings = 0
                zero_vector = google_generator._get_zero_vector()
                for i, chunk in enumerate(google_embedded_chunks):
                    if chunk.embedding and len(chunk.embedding) == google_generator.vector_dim:
                         if chunk.embedding != zero_vector:
                            valid_embeddings += 1
                    if i < 5:
                         embedding_preview = chunk.embedding[:5] if chunk.embedding else None
                         is_zero = " (Zero Vector)" if chunk.embedding == zero_vector else ""
                         logger.info(f"  Chunk {i}: '{chunk.text[:30]}...' -> Embedding Dim: {len(chunk.embedding) if chunk.embedding else 'N/A'}{is_zero}, Preview: {embedding_preview}...")
                logger.info(f"  Total valid (non-zero) Google embeddings generated: {valid_embeddings}/{len(google_embedded_chunks)}")
            except EmbeddingError as e:
                 logger.error(f"Google Embedding generation failed: {e}")
            except Exception as e:
                 logger.exception(f"An unexpected error occurred during Google testing: {e}", exc_info=True)
        else:
            logger.warning("Skipping Google test: Google API Key not set or found.")

        # Test OpenAI (if key is available)
        if settings.llm.openai_api_key:
            logger.info("\n--- Testing OpenAI Embeddings ---")
            openai_test_chunks = [TextChunk(text=c.text) for c in test_chunks_data]
            try:
                openai_generator = EmbeddingGenerator(provider='openai', model_name='text-embedding-3-small', batch_size=10)
                start_time = time.time()
                openai_embedded_chunks = openai_generator.embed_text_chunks(openai_test_chunks)
                end_time = time.time()
                logger.info(f"Generated {len(openai_embedded_chunks)} OpenAI embeddings in {end_time - start_time:.2f} seconds.")
                valid_embeddings = 0
                zero_vector = openai_generator._get_zero_vector()
                for i, chunk in enumerate(openai_embedded_chunks):
                     if chunk.embedding and len(chunk.embedding) == openai_generator.vector_dim:
                         if chunk.embedding != zero_vector:
                            valid_embeddings += 1
                     if i < 5:
                         embedding_preview = chunk.embedding[:5] if chunk.embedding else None
                         is_zero = " (Zero Vector)" if chunk.embedding == zero_vector else ""
                         logger.info(f"  Chunk {i}: '{chunk.text[:30]}...' -> Embedding Dim: {len(chunk.embedding) if chunk.embedding else 'N/A'}{is_zero}, Preview: {embedding_preview}...")
                logger.info(f"  Total valid (non-zero) OpenAI embeddings generated: {valid_embeddings}/{len(openai_embedded_chunks)}")
            except EmbeddingError as e:
                 logger.error(f"OpenAI Embedding generation failed: {e}")
            except Exception as e:
                 logger.exception(f"An unexpected error occurred during OpenAI testing: {e}", exc_info=True)
        else:
            logger.warning("Skipping OpenAI test: OpenAI API Key not set or found.")

    except EmbeddingError as e:
        logger.error(f"Initialization or general embedding error: {e}")
    except Exception as e:
        logger.exception(f"An unexpected error occurred during setup or testing: {e}", exc_info=True)